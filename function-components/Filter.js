...
const Filter = ({ onSearch, fields = [], intlPrefix = '',
  ...
  function handleSearch() {
      ...
      console.log('handling search, data:', data);
      onSearch(data);
  }
  ...
  const createByFields = (fields) => fields.length>0 &&
     fields.map((item, index) => {
        if(typeof(item) === 'object') {
            ...
            return (
              <span key={`${index}`}>
                <span style={{ marginTop: '5px'}}>
                  <FormattedMessage id={`${intlPrefix}${item.name}`} />:
                </span>
                {getFieldDecorator(item.name, {
                    initialValue: ''
                  })(
                    <Select style={{ width: '150px', margin: '0px 5px 10px 5px' }}>
            ...
            return (
              <span key={`${index}`}>
                ...
                {getFieldDecorator(item.name, {
                    ...
                    <Select style={{ width: '150px', margin: '0px 10px 10px 5px' }}>
                      <Select.Option value=""></Select.Option>
                      {opts()}
                    </Select>
                  )}
          ...
          return (
            <Col span={5} style={{ paddingRight: '10px'}} key={`${index}`}>
                  {getFieldDecorator(item, {
                    initialValue: ''
                  })(
                    <Input addonBefore={<FormattedMessage id={`${intlPrefix}${item}`} />} />
                  )}
            </Col>
          )
        }
  ...
  const createSearchBtn = () => {
    if(fields.length>0  && onSearch) {
      return (
        <span>
          <Button onClick={() => handleSearch()}  type="primary" style={{marginTop: '2px',marginLeft: '1.5em'}}>搜索</Button>
